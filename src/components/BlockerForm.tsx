...
export default function BlockerForm({ taskId, allTasks, onAdd }: Props) {
  const [blockerType, setBlockerType] = useState<'task' | 'date'>('task');
  const [selectedTask, setSelectedTask] = useState<string>('');
  const [selectedDate, setSelectedDate] = useState('');

  const otherTasks = allTasks.filter(t => t.id !== taskId && t.completedAt == null);
  ...
  const handleAdd = async () => {
    if (blockerType === 'task' && selectedTask) {
      ...
      await onAdd({ blockedUntilDate: new Date(selectedDate).toISOString() });
      setSelectedDate('');
    }
  };
  ...
  return (
    <div className="blocker-form">
      <select value={blockerType} onChange={e => setBlockerType(e.target.value as 'task' | 'date')}>
      ...
      <button className="btn btn-sm btn-primary" onClick={handleAdd}>Add Blocker</button>
    </div>
  );
}
